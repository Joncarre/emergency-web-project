/**
 * Isla de alta de Bienes. El identificador (matrícula / nº de serie) es la clave:
 * obligatorio y único. Comprobamos coincidencia exacta en vivo y, al publicar,
 * si el identificador ya existe NO se duplica: se muestra la ficha existente.
 */
import { useEffect, useState } from 'preact/hooks';
import type { GoodCard, GoodDirection, GoodType } from '@/lib/engine/types';
...
export default function GoodCreateForm({ lang, defaultZone }: Props) {
  const t = useTranslations(lang);

  const [direction, setDirection] = useState<GoodDirection>('found');
  const [goodType, setGoodType] = useState<GoodType>('vehicle');
  ...
  const [created, setCreated] = useState<{ card: GoodCard; manageUrl: string } | null>(null);
  const [matched, setMatched] = useState<GoodCard | null>(null);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    if (naturalId.trim().length < 2) {
      setMatches([]);
      return;
    }
    const id = setTimeout(async () => {
      try {
        const res = await fetch('/api/bienes/similar', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ naturalId: naturalId || null, goodType, zone: zone || null }),
        });
  ...
  }, [naturalId, goodType, zone]);

  async function submit(e: Event) {
    e.preventDefault();
    if (naturalId.trim().length < 2) {
      setError(t('goods.field.naturalId'));
      return;
    }
    setSubmitting(true);
    setError(null);
  ...
  if (matched) {
    return (
      <div class="rounded-[var(--radius-md)] border border-warning/40 bg-warning-soft p-5">
        <h2 class="text-lg font-semibold text-fg">{t('goods.new.matched.title')}</h2>
        <p class="mt-2 text-sm text-fg-secondary">{t('goods.new.matched.body')}</p>
        <a href={`/bienes/${matched.id}`} class="mt-4 inline-block text-sm font-medium text-primary hover:underline">
          {t('goods.new.matched.view')} →
        </a>
      </div>
    );
  }
    ...
    return (
      <div class="rounded-[var(--radius-md)] border border-border bg-surface p-5">
        <div class="flex items-center gap-2 text-success">
          ...
          <h2 class="text-lg font-semibold text-fg">{t('goods.new.success.title')}</h2>
        </div>
        <p class="mt-2 text-sm text-fg-tertiary">{t('goods.new.success.body')}</p>
        ...
        <a href={`/bienes/${created.card.id}`} class="mt-4 inline-block text-sm font-medium text-primary hover:underline">
          {t('goods.new.view')} →
        </a>
      </div>
    );
  ...
  return (
    <form onSubmit={submit} class="space-y-5">
      <fieldset class="space-y-2">
        <legend class={labelCls + ' mb-1'}>{t('goods.field.direction')}</legend>
        {(['found', 'lost'] as GoodDirection[]).map((d) => (
          <label key={d} class={'flex cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border p-3 transition-colors ' + (direction === d ? 'border-primary bg-primary-soft' : 'border-border bg-surface hover:bg-surface-2')}>
            ...
            <span>
              <span class="block text-sm font-medium text-fg">{t(`goods.direction.${d}` as UIKey)}</span>
              <span class="block text-xs text-fg-tertiary">{t(`goods.new.direction.${d}.desc` as UIKey)}</span>
            </span>
          </label>
        ))}
      </fieldset>
      ...
      <div class="space-y-1.5">
        <label for="type" class={labelCls}><span>{t('goods.field.type')}</span></label>
        <select id="type" class="select" value={goodType} onChange={(e) => setGoodType((e.target as HTMLSelectElement).value as GoodType)}>
          {TYPES.map((ty) => <option key={ty} value={ty}>{t(`goods.type.${ty}` as UIKey)}</option>)}
        </select>
      </div>

      <div class="space-y-1.5">
        <label for="nid" class={labelCls}>
          <span>{t('goods.field.naturalId')}</span>
          <span class="text-xs text-danger" aria-hidden="true">*</span>
        </label>
        <input id="nid" class="input font-mono uppercase" required maxLength={40} value={naturalId} onInput={(e) => setNaturalId((e.target as HTMLInputElement).value)} />
      </div>

      {showSimilar && (
        <div class="rounded-[var(--radius-sm)] border border-warning/40 bg-warning-soft p-3">
          <p class="text-sm font-semibold text-fg">{t('goods.new.similar.title')}</p>
          <p class="mt-0.5 text-xs text-fg-tertiary">{t('goods.new.similar.body')}</p>
          <ul class="mt-2 space-y-1.5">
            {matches.map((m) => (
              <li key={m.card.id}>
                <a href={`/bienes/${m.card.id}`} class="flex items-center justify-between gap-2 rounded bg-bg/50 px-3 py-2 text-sm hover:bg-bg">
                  <span class="font-mono font-medium text-fg">{m.card.naturalId}</span>
                  <span class="text-xs text-fg-tertiary">
                    {m.reason === 'natural_id' ? t('goods.new.similar.id') : m.card.zone} · {t('goods.new.similar.use')} →
                  </span>
                </a>
              </li>
            ))}
      ...
      <button type="submit" disabled={submitting} class="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-primary px-5 text-base font-medium text-on-accent transition-[transform,background-color] duration-150 hover:bg-primary-hover active:scale-[0.98] disabled:opacity-50">
        {submitting ? t('common.loading') : t('goods.new.submit')}
      </button>
    </form>
  );
}
