/**
 * POST /api/personas/:id/transition — cambio de estado (creador con token o
...
export const POST: APIRoute = async ({ params, request }) => {
  ...
  const limit = rateLimit(`people:transition:${ip}`, 30, 10 * 60_000);
  ...
  const back = `/personas/gestionar?id=${id}&token=${encodeURIComponent(token)}`;
  if (!parsed.success) return redirect(`${back}&err=validation`);

  const repo = getRepository();
  try {
    await repo.transitionPerson(id, parsed.data.to, {
      manageToken: token,
