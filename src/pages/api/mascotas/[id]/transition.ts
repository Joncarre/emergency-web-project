/**
 * POST /api/mascotas/:id/transition — cambio de estado (creador con token o
...
export const POST: APIRoute = async ({ params, request }) => {
  ...
  const limit = rateLimit(`pets:transition:${ip}`, 30, 10 * 60_000);
  ...
  const back = `/mascotas/gestionar?id=${id}&token=${encodeURIComponent(token)}`;
  if (!parsed.success) return redirect(`${back}&err=validation`);

  const repo = getRepository();
  try {
    await repo.transitionPet(id, parsed.data.to, {
      manageToken: token,
    ...
    return redirect(`/mascotas?err=not_found`);
  }
