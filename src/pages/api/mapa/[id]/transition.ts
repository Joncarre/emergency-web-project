/**
 * POST /api/mapa/:id/transition — retirar/reactivar un reporte (creador con
 * token o moderación). Formulario nativo desde la página de gestión.
 */
...
export const POST: APIRoute = async ({ params, request }) => {
  ...
  const limit = rateLimit(`map:transition:${ip}`, 30, 10 * 60_000);
  ...
  const back = `/mapa/gestionar?id=${id}&token=${encodeURIComponent(token)}`;
  if (!parsed.success) return redirect(`${back}&err=validation`);

  const repo = getRepository();
  try {
    await repo.transitionMapReport(id, parsed.data.to, {
      manageToken: token,
    ...
    return redirect(`/mapa?err=not_found`);
  }
